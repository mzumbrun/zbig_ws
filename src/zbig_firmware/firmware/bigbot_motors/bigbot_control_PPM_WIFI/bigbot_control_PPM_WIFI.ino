...
double speed = 1500.;
double max_rads_per_sec = 7.8;  // 7.8 corresponds to that in ROS for 0.5 m/s for bigbot
...
void setup() {
 ...
 // Motor.begin(&wheel_meas_vel, &wheel_cmd, &wheel_cmd_vel, Kp, Ki, Kd);
  ...
  if (digitalRead(motor_select) == HIGH) {
    ...
    Udp.begin(leftPort);
    is_right = false;
    max_pos_speed = 2200;  // corresponds to max rad/s for LEFT motor to match max provided by ROS
    max_neg_speed = 1000;
    Kp = KpL;
    Ki = KiL;
    Kd = KdL;
...
void loop() {
  ...
  //  Serial.print(",");
   // Serial.println(encoder_count_);
    ...
    dtostrf(wheel_meas_vel, PACKET_SIZE, 1, message_string);
